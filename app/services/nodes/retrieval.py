"""
app/services/nodes/retrieval.py

Provides the retrieval subgraph nodes: router, qdrant_search, neo4j_search, synthesizer.
Moved from src/nodes/retrieval.py — import paths updated for the new structure.
All query logic (Text2Cypher, fallback, synthesizer) is preserved exactly.
"""
...
# ---------------------------------------------------------------------------
# LLM helpers (replaces src.services.llm imports)
# ---------------------------------------------------------------------------

def get_llm_model(temperature: float = 0, model: str = "gpt-4o-mini") -> ChatOpenAI:
    """Standard LLM for routing and Cypher generation."""
...
class FallbackKeyword(BaseModel):
    keyword: str = Field(
        description="A single core english noun extracted or translated from the query"
    )
...
def router_node(state: dict) -> dict:
    """Route the query to the appropriate search backend(s)."""
    query = state["query"]
    prompt = f"""
Given the user query, decide if we should use vector search (for broad semantic similarity), \
graph search (for finding specific relationships between roles, departments, concepts, or exact policies), \
or both.
...
def qdrant_search_node(state: dict) -> dict:
...
def neo4j_search_node(state: dict) -> dict:
    """Generate and execute a Cypher query via Text2Cypher, with fallback."""
    query = state["query"]
...
"""
    ...
    try:
        ...
        if not records:
            ...
            kw_prompt = (
                f"Extract exactly one core single-word English noun that represents "
                f"the main entity in this query (translate to English if necessary): {query}"
            )
            try:
                ...
                if keyword:
                    fallback_cypher = (
                        f"MATCH (n)-[r]-(m) WHERE toLower(n.id) CONTAINS toLower('{keyword}') "
                        f"RETURN labels(n)[0] as FromType, n.id as FromName, "
                        f"type(r) as Relation, labels(m)[0] as ToType, m.id as ToName LIMIT 15"
                    )
...
def synthesizer_node(state: dict) -> dict:
    """Merge vector and graph contexts into a coherent body of knowledge."""
